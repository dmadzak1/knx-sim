...
**Current status: M3 done.** Update the line below as work progresses.

> STATUS: M3 done (F-BUS-1..7, F-DEV-0, F-DEV-1, F-DEV-6). knx_sim/devices/group_object.py: GroupObject (mutable, sync, no I/O) + GroupObjectFlags (C/R/W/T/U) -- bridges DPT-decoded values and Telegram's int|bytes payload shape via to_payload()/apply_payload(). knx_sim/devices/device.py: Device base -- send()/respond() (GroupValueResponse, answers Reads)/transmit() (GroupValueWrite, spontaneous Transmit-flag behavior) via an injected async send callable (bound by the bus at registration, not at construction); handle_group_write/handle_group_read override points, the latter's default already satisfying F-DEV-8 for every device. knx_sim/bus/router.py: Bus -- device/group-address registry, asyncio.PriorityQueue with an explicit urgency-rank map (System>Urgent>Normal>Low; NOT the same order as Priority's cEMI wire-bit values from M2), one background task serializing telegrams with a configurable propagation delay, full C/R/W/U flag-gated routing, rolling telegram log with DPT-resolved decoded values, subscribe/unsubscribe monitor API, queue.join()-based settle-waiting for tests. knx_sim/devices/switch.py: SwitchActuator + WallSwitch. 231 tests passing (including tests/test_integration.py, the switch-presses-lamp acceptance test from SPEC.md), ruff+mypy strict clean. Next action: M4 -- KNXnet/IP discovery + routing (F-IP-1, F-IP-2, F-IP-5) on asyncio UDP; no guide steps beyond this point, design together as needed.
