...
**Current status: M2 done.** Update the line below as work progresses.

> STATUS: M2 done. knx_sim/cemi/address.py: IndividualAddress (4+4+8 bit) and GroupAddress (5+3+8 bit), frozen dataclasses, string parse/format, to_knx/from_knx, hashable. knx_sim/cemi/frame.py: Telegram dataclass (payload: int|bytes|None -- int=inline <=6-bit merged into APCI, bytes=appended, None=GroupValueRead), MessageCode/Priority/Service enums, build_cemi/parse_cemi, typed ParseError with specific messages for every malformed-input case. Scope: only L_Data.req/ind/con, only GroupValueRead/Write/Response, only unnumbered group communication (matches decided architecture -- no additional-info blocks, no individual-address destinations, no extended/LTE addressing). Verified against 4 byte-exact fixtures generated from xknx (scripts/generate_cemi_fixtures.py -> tests/fixtures/cemi/known_frames.json), one of which matches the hand-derived docs/notes/cemi.md example exactly. 185 tests passing, ruff+mypy strict clean. Next action: M3 -- in-process bus + switch/lamp devices (GroupObject, Device base, bus router, then integration test "press switch -> lamp status telegram appears"), per docs/GUIDE.md Part 5 pointers (no detailed step-by-step in the guide beyond this point -- use judgment / ask the developer).
